...
$DRIVER_BUILD_DIR/zfs-${DRIVER_V_PKG}/configure --prefix=/usr --libdir=/lib/aarch64-linux-gnu --sysconfdir=/etc
make -j$(nproc --all)
...
rm -rf $DRIVER_PACKAGE_DIR/usr/include $DRIVER_PACKAGE_DIR/usr/src $DRIVER_PACKAGE_DIR/etc/sudoers.d $DRIVER_PACKAGE_DIR/lib/aarch64-linux-gnu/pkgconfig
find $DRIVER_PACKAGE_DIR/lib/aarch64-linux-gnu -type f \( -name "*.a" -o -name "*.la" \) -delete
