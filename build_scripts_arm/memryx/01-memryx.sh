...
rm -rf $DRIVER_BUILD_DIR/mxa_manager/aarch64/lib/firmware \
  $DRIVER_BUILD_DIR/mxa_manager/aarch64/lib/systemd \
  $DRIVER_BUILD_DIR/mxa_manager/aarch64/usr/include \
  $DRIVER_BUILD_DIR/mxa_manager/aarch64/usr/src \
  $DRIVER_BUILD_DIR/mxa_manager/aarch64/opt \
  $DRIVER_BUILD_DIR/mxa_manager/aarch64/etc/ld.so.conf.d \
  $DRIVER_BUILD_DIR/mxa_manager/aarch64/etc/modules-load.d

find $DRIVER_BUILD_DIR/mxa_manager/aarch64 -type f \( -name "*.h" -o -name "*.a" -o -name "*.pc" -o -name "*.cmake" \) -delete
find $DRIVER_BUILD_DIR/mxa_manager/aarch64 -type d -empty -delete

cp -R $DRIVER_BUILD_DIR/mxa_manager/aarch64/* $DRIVER_PACKAGE_DIR/
